...
const HistomicsTrainingAnalysis = () => {
  ...
  const [showSurvivalPlot, setShowSurvivalPlot] = useState(false);
...
useEffect(() => {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
  	if (segmentationComplete && selectedSlide) {
    	  checkForExistingModel();
      }
   }, [segmentationComplete, selectedSlide]);
  ...
  const updateOverlay = () => {
    ...
    timeoutId = window.setTimeout(() => {
      if (showPredictions && predictions.length > 0) {
        ...
        // ADD: Clear heatmap when showing predictions
        ...
        // ADD: Clear heatmap when showing nuclei
        if (heatmapOverlayRef) {
          viewer.removeOverlay(heatmapOverlayRef);
          setHeatmapOverlayRef(null);
        }
...
//}, [viewer, segmentationComplete, nucleiData, showSegmentation, selectedSamples, predictions, showPredictions]);
...
const fetchSurvivalData = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/survival/km`);
    const result = await response.json();
    if (result.success) {
      setSurvivalData(result);
      setShowSurvivalPlot(true);
    } else {
      alert('Survival analysis failed: ' + result.error);
    }
  } catch (error) {
    alert('Error fetching survival data: ' + error);
  } finally {
...
const handleUseExistingModel = async () => {
  ...
  // Validate model matches slide
  if (trainedModelName && trainedModelName !== selectedSlide.name) {
    const confirm = window.confirm(
      `Warning: The loaded model was trained on '${trainedModelName}' ` +
      `but you're trying to use it on '${selectedSlide.name}'. ` +
      `This may produce invalid results. Continue anyway?`
    );
  ...
  try {
    ...
    console.log('Using existing model, job:', result.job_id);
  ...
  <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
    <div className="mb-8">
      <h1 className="text-4xl font-bold text-gray-800 mb-3">
         HIMLT: Hybrid Interactive Machine Learning Tool for Histopathology Image Analysis. | Survival Analysis
      </h1>
    </div>
	...
	{/* Slide Selector */}
      ...
      {predictions.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg p-4">
          <h3 className="font-semibold mb-3">Survival Analysis</h3>
          <button
            onClick={() => {
              if (showSurvivalPlot) {
                setShowSurvivalPlot(false);
              } else {
                fetchSurvivalData();
              }
            }}
            disabled={loadingSurvival}
            className="w-full px-4 py-3 rounded-xl font-semibold text-white bg-blue-500 hover:bg-blue-600 transition-all disabled:opacity-50"
          >
            {loadingSurvival ? 'Loading...' : showSurvivalPlot ? 'Hide Survival Analysis' : 'View Survival Analysis'}
          </button>
...
{segmentationComplete && !isTraining && (
  <div className="bg-white rounded-xl shadow-lg p-6">
    ...
    {trainedModelName && (
      <>
        <div className="p-2 bg-green-50 rounded text-sm text-green-700 mb-3">
          Current model: {trainedModelName}
        </div>

        <button
          onClick={handleUseExistingModel}
          className="w-full mb-2 px-6 py-3 rounded-xl font-semibold text-white bg-purple-500 hover:bg-purple-600 transition-all"
        >
          <Brain className="inline mr-2" size={20} />
          Run Predictions
        </button>
    ...
    >
      {showModelManager 
        ? `Hide Available Models (${availableModels.length})` 
        ...
        {availableModels.length === 0 ? (
          ...
          <div className="divide-y">
            {availableModels.map((model) => (
              <div key={model.slide_name} className="p-3 hover:bg-gray-50">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <p className="font-medium text-sm">{model.slide_name}</p>
                    <p className="text-xs text-gray-500">
                      {model.size_mb} MB • {model.created}
                    </p>
                    {!model.is_valid && (
                      <p className="text-xs text-red-600">Corrupted</p>
                    )}
                    {model.slide_name === selectedSlide?.name ? (
                      <p className="text-xs text-green-600">✓ Active Model</p>
                    ) : (
                      <p className="text-xs text-gray-400">
                         Slide-specific model
                      </p>
                    )}
                  </div>
                </div>
	...
	{/* RIGHT VIEWER */}
      <div className="lg:col-span-3">
        <div className="bg-white rounded-xl shadow-lg p-8 h-full min-h-[600px]">
          ...
          {showSurvivalPlot && survivalData && (() => {
            const W = 680, H = 270, padL = 70, padR = 190, padT = 20, padB = 40;
            const plotW = W - padL - padR;
            const plotH = H - padT - padB;

            const allTimes = [
              ...survivalData.high_group.curve.map((p: number[]) => p[0]),
              ...survivalData.low_group.curve.map((p: number[]) => p[0])
            ];
            const maxTime = Math.max(...allTimes);

            const toX = (t: number) => padL + (t / maxTime) * plotW;
            const toY = (s: number) => padT + (1 - s) * plotH;

            const stepPoints = (curve: number[][]) => {
              let pts = '';
              for (let i = 0; i < curve.length; i++) {
                const x = toX(curve[i][0]);
                const y = toY(curve[i][1]);
                if (i === 0) {
                  pts += `${x},${y}`;
                } else {
                  const prevY = toY(curve[i - 1][1]);
                  pts += ` ${x},${prevY} ${x},${y}`;
                }
              }
              return pts;
            ...
            return (
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                <h4 className="font-semibold text-lg mb-1 text-center">Kaplan-Meier Survival Curve</h4>
                <p className="text-xs text-center text-gray-500 mb-3">Median cutoff: {survivalData.median_cutoff.toFixed(2)} | p-value: {survivalData.p_value}</p>
                <svg width="100%" viewBox={`0 0 ${W} ${H}`} className="border rounded bg-white">
                  {/* Axes */}
                  <line x1={padL} y1={padT} x2={padL} y2={padT + plotH} stroke="black" strokeWidth="1.5"/>
                  <line x1={padL} y1={padT + plotH} x2={padL + plotW} y2={padT + plotH} stroke="black" strokeWidth="1.5"/>
                  {/* Y label */}
                  <text x="12" y={padT + plotH / 2} fontSize="11" textAnchor="middle" transform={`rotate(-90,12,${padT + plotH / 2})`}>Survival Probability</text>
                  {/* X label */}
                  <text x={padL + plotW / 2} y={H - 2} fontSize="11" textAnchor="middle">Time (days)</text>
                  {/* Y ticks */}
                  {[0, 0.25, 0.5, 0.75, 1.0].map(v => (
                    <g key={v}>
                      <line x1={padL - 4} y1={toY(v)} x2={padL} y2={toY(v)} stroke="black" strokeWidth="1"/>
                      <text x={padL - 6} y={toY(v) + 4} fontSize="9" textAnchor="end">{v.toFixed(2)}</text>
                    </g>
                  ))}
                  {/* X ticks */}
                  {[0, Math.round(maxTime / 2), Math.round(maxTime)].map(v => (
                    <g key={v}>
                      <line x1={toX(v)} y1={padT + plotH} x2={toX(v)} y2={padT + plotH + 4} stroke="black" strokeWidth="1"/>
                      <text x={toX(v)} y={padT + plotH + 14} fontSize="9" textAnchor="middle">{v}</text>
                    </g>
                  ))}
                  {/* High group - Red */}
                  <polyline points={stepPoints(survivalData.high_group.curve)} fill="none" stroke="red" strokeWidth="2.5"/>
                  {/* Low group - Blue */}
                  <polyline points={stepPoints(survivalData.low_group.curve)} fill="none" stroke="blue" strokeWidth="2.5"/>
                  {/* Legend - outside plot on the right */}
                  <rect x={W - padR + 14} y={padT + 10} width="18" height="3" fill="red"/>
                  <text x={W - padR + 36} y={padT + 14} fontSize="10">High pct_pos_nuclei (n={survivalData.high_group.n})</text>
                  <rect x={W - padR + 14} y={padT + 26} width="18" height="3" fill="blue"/>
                  <text x={W - padR + 36} y={padT + 30} fontSize="10">Low pct_pos_nuclei (n={survivalData.low_group.n})</text>
                </svg>
              </div>
            );
          })()}
