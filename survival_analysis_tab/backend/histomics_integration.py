...
class HistomicsMLBridge:
    """Bridge between Flask API and HistomicsML's model_server.py"""
    ...
    def submit_training_job(self, dataset_id, slide_id, slide_name, annotations, target='train_heatmap'):
        ...
        Args:
            ...
            slide_name: Name of slide (e.g., 'TCGA-02-0001-01Z-00-DX1.tif')
    ...
    def submit_prediction_job(self, dataset_id, slide_id, slide_name, model_name=None):
        ...
        Args:
            ...
            model_name: Name of model to use (if None, uses slide_name for backward compatibility)
        """
        try:
            # Get dataset info
            conn = mysql.connector.connect(**self.db_config)
            ...
            # Use provided model_name or default to slide_name for backward compatibility
            if model_name is None:
                model_name = slide_name
                logger.info(f"No model specified, using slide name: {model_name}")
            else:
            ...
            logger.info(f"[BRIDGE] Predicting slide: {slide_name} using model: {model_name}")
            if model_name != slide_name:
              logger.warning(f"[BRIDGE] Transfer Learning: Applying pre-trained model to new slide")
