...
# KEEP Redis endpoints for future real-time segmentation (commented out for now)
"""
@app.route("/api/segmentation/region", methods=['POST'])
def segment_region():
    # Interactive segmentation endpoint using Redis queue
    uid = str(uuid.uuid4())
    ...
    print("Segmentation request: " + uid + " for slide " + str(slide_id))
    
    d = dict(
        id=uid,
        uid=uid,
        action='segment_region',
        slide_id=slide_id,
    ...
    while True:
        ...
        if output is not None:
            ...
            db.delete(uid)
            print("Segmentation complete: " + uid)
            return jsonify(data)
        
        if time.time() - timeout_start > s.SEGMENTATION_TIMEOUT:
            return jsonify({"success": False, "error": "Segmentation timeout"}), 504
...
def annotate_samples():
    ...
    try:
        ...
        # TODO: Optionally save annotations to database for tracking
...
MODEL_SERVER_CHECKPOINT_DIR = '/opt/new_histomics/checkpoints'
...
def predict_with_existing_model():
    ...
    try:
        ...
        if not all([dataset_id, slide_id, model_name]):
            return jsonify({
                'success': False,
                'error': 'Missing required fields: dataset_id, slide_id, model_name'
            }), 400
        ...
        # Get model name from request (allows cross-slide prediction)
        model_name = data.get('model_name')
        if not model_name:
            return jsonify({
                 'success': False,
        ...
        if not os.path.exists(model_path):
            return jsonify({
                'success': False,
                'error': f'No trained model found: {model_name}.pkl. Please train or upload a model first.'
            }), 404

        # Log cross-slide prediction warning
        if model_name != slide_name:
             logger.warning(f"Transfer Learning: Using model '{model_name}' for slide '{slide_name}'")
        else:
            logger.info(f"Same-slide prediction: Using model '{model_name}' for slide '{slide_name}'")
        
        # Submit prediction job (no training, just predict)
        ...
        logger.info(f'Submitted predict_only job: {job_id} for slide: {slide_name}')

        return jsonify({
            'success': True,
            'job_id': job_id,
            'message': f'Prediction job submitted using existing model for {slide_name}'
        })
...
@app.route('/api/survival/km', methods=['GET'])
def get_km_survival():
    """
    Compute Kaplan-Meier survival curves split by median pct_pos_nuclei.
    Returns two curves (High / Low group) and log-rank p-value.
    """
    try:
        import numpy as np
        ...
        cursor = connection.cursor(dictionary=True)
        cursor.execute("""
            SELECT patient_id, OS_time_days, OS_event, pct_pos_nuclei
            FROM patient_clinical_survival
            WHERE pct_pos_nuclei IS NOT NULL
              AND OS_time_days IS NOT NULL
              AND OS_event IS NOT NULL
        """)
        ...
        if len(rows) < 2:
            return jsonify({'success': False, 'error': 'Not enough patient data'}), 400

        # Split by median pct_pos_nuclei
        median_cutoff = float(np.median([r['pct_pos_nuclei'] for r in rows]))

        high_group = [r for r in rows if r['pct_pos_nuclei'] >= median_cutoff]
        low_group  = [r for r in rows if r['pct_pos_nuclei'] <  median_cutoff]

        def km_curve(group):
            times  = [r['OS_time_days'] for r in group]
            events = [r['OS_event']     for r in group]
            sorted_pairs = sorted(zip(times, events))
            curve = [(0.0, 1.0)]
            survival = 1.0
            n = len(sorted_pairs)
            for i, (t, e) in enumerate(sorted_pairs):
                at_risk = n - i
                if e == 1:
                    survival *= (1 - 1 / at_risk)
                curve.append((float(t), round(float(survival), 6)))
            return curve

        def log_rank_p(group1, group2):
            times1  = [r['OS_time_days'] for r in group1]
            events1 = [r['OS_event']     for r in group1]
            times2  = [r['OS_time_days'] for r in group2]
            events2 = [r['OS_event']     for r in group2]

            all_event_times = sorted(set(
                t for t, e in zip(times1 + times2, events1 + events2) if e == 1
            ))

            O, E = 0.0, 0.0
            for t in all_event_times:
                n1 = sum(1 for ti in times1 if ti >= t)
                n2 = sum(1 for ti in times2 if ti >= t)
                n  = n1 + n2
                if n == 0:
                    continue
        ...
        p_value = log_rank_p(high_group, low_group)
        ...
        return jsonify({
            ...
            'high_group': {
                ...
                'n': len(high_group),
                'curve': km_curve(high_group)
            },
            'low_group': {
                'name': 'Low pct_pos_nuclei',
                'n': len(low_group),
                'curve': km_curve(low_group)
            }
        })

    except Exception as e:
        logger.error(f'Error computing KM survival: {str(e)}')
        return jsonify({'success': False, 'error': str(e)}), 500
